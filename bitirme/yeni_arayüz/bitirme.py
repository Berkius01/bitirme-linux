...
def init_stream():
    ...
    streamIn = audio.open(format=FORMATIN, channels=CHANNELS,
                          rate=RATE, input=True, output=True, input_device_index=index,
...
def save_text_and_summary(text):
    ...
    text_file = open(str(os.getcwd())+'\\original.txt', "w")
    ...
    sumary_file = open(str(os.getcwd())+'\\summary.txt',"a")
    # Summarize text
